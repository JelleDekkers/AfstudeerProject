...
Shader "Shader Forge/InteractablesEnv" {
    Properties {
        ...
        _coloroutline ("color outline", Color) = (0.8455882,1,1,1)
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
...
/////// Diffuse:
                ...
                float4 node_3984 = tex2D(_Diffuse,TRANSFORM_TEX(i.uv0, _Diffuse));
                float node_5798 = 10.0;
                float3 diffuseColor = ((node_3984.rgb*_Maincolor.rgb)+(_coloroutline.rgb*floor(pow(1.0-max(0,dot(normalDirection, viewDirection)),2.0) * node_5798) / (node_5798 - 1)));
