...
public class SmoothRandom {
    ...
    public static Vector3 GetVector3Hybrid(float speed, float timeMultiplier) {
        float time = Time.time * timeMultiplier * speed;
        return new Vector3(Get().HybridMultifractal(time, 15.73F, 0.58F), Get().HybridMultifractal(time, 63.94F, 0.58F), Get().HybridMultifractal(time, 0.2F, 0.58F));
    }

    public static Vector3 GetVector3Ridged(float speed, float timeMultiplier) {
        float time = Time.time * timeMultiplier * speed;
        float gain = 1;
        return new Vector3(Get().RidgedMultifractal(time, 15.73F, 0.58F, gain), Get().RidgedMultifractal(time, 63.94F, 0.58F, gain), Get().RidgedMultifractal(time, 0.2F, 0.58F, gain));
    }

    public static Vector3 GetVector3Brownian(float speed, float timeMultiplier) {
        float time = Time.time * timeMultiplier * speed;
        return new Vector3(Get().BrownianMotion(time, 15.73F), Get().BrownianMotion(time, 63.94F), Get().BrownianMotion(time, 0.2F));
    }
